...
export class RegionSelector {
    ...
    onClick(event) {
        ...
        if (this.selectionType === 'polygon') {
            ...
            if (this.tempPoints.length > 0) {
                ...
                const distance = Math.sqrt((theta - lastPoint.theta) ** 2 + (phi - lastPoint.phi) ** 2);
                if (distance < 0.1) {
                    this.finishSelection();
                    return;
                }
     ...
     * 检查点是否在区域内
     */
    isPointInRegion(point, regionId) {
     ...
     * 点在矩形内判断
     */
    isPointInRectangle(point, points) {
        if (points.length < 2)
            return false;
     ...
     * 点在圆形内判断
     */
    isPointInCircle(point, points) {
        if (points.length < 2)
            return false;
        const center = points[0];
        const edge = points[1];
        const radius = Math.sqrt((edge.theta - center.theta) ** 2 + (edge.phi - center.phi) ** 2);
        const distance = Math.sqrt((point.theta - center.theta) ** 2 + (point.phi - center.phi) ** 2);
     ...
     * 渲染矩形
     */
    renderRectangle(points, color, strokeWidth) {
        if (points.length < 2)
            return;
     ...
     * 渲染圆形
     */
    renderCircle(points, color, strokeWidth) {
        if (points.length < 2)
            return;
     ...
     * 渲染多边形
     */
    renderPolygon(points, color, strokeWidth) {
        if (points.length < 3)
            return;
        this.ctx.beginPath();
     ...
     * 渲染临时区域
     */
    renderTempRegion(points) {
        this.renderRectangle(points, '#ffff00', 2);
    }
    /**
     * 渲染临时圆形
     */
    renderTempCircle(center, edge) {
