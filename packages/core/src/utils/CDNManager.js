...
export class CDNManager {
    ...
    getUrl(path, useFallback = false) {
        ...
        const fullPath = this.config.pathPrefix
            ? `${this.config.pathPrefix}/${cleanPath}`
            : cleanPath;
        // 选择CDN
     ...
     * 更新统计
     */
    updateStats(cdn, success, latency) {
     ...
     * 加载一个小文件测试所有CDN的可用性
     */
    async warmup(testPath = 'favicon.ico') {
        const allCDNs = [this.config.primary, ...this.config.fallbacks];
