...
export class PostProcessing {
    ...
    addBloom() {
        ...
        this.bloomPass = new UnrealBloomPass(new THREE.Vector2(size.x, size.y), this.options.bloom.strength, this.options.bloom.radius, this.options.bloom.threshold);
     ...
     */
    addAntialiasing() {
        if (!this.composer || this.options.antialiasing === 'none')
            return;
    ...
    setDepthOfFieldParams(params) {
        ...
        if (params.focus !== undefined) {
            // @ts-expect-error - 第三方库uniforms类型
