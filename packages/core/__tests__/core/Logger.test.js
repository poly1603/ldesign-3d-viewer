...
describe('Logger', () => {
    ...
    describe('日志级别', () => {
        it('应该正确输出不同级别的日志', () => {
            ...
            expect(consoleSpy.debug).toHaveBeenCalledWith(expect.stringMatching(/\[TestLogger\].*Debug message/));
            expect(consoleSpy.info).toHaveBeenCalledWith(expect.stringMatching(/\[TestLogger\].*Info message/));
            expect(consoleSpy.warn).toHaveBeenCalledWith(expect.stringMatching(/\[TestLogger\].*Warning message/));
    ...
    describe('日志格式', () => {
        ...
        it('应该包含Logger名称', () => {
            logger.info('Test');
            expect(consoleSpy.info).toHaveBeenCalledWith(expect.stringMatching(/^\[TestLogger\]/));
        });
        it('应该支持data参数', () => {
            const obj = { key: 'value' };
            logger.info('Message', obj);
            expect(consoleSpy.info).toHaveBeenCalledWith(expect.stringMatching(/\[TestLogger\].*Message/), obj);
        });
    });
    describe('日志历史', () => {
        it('应该记录日志历史', () => {
    ...
    describe('错误对象处理', () => {
        it('应该能够记录Error对象', () => {
            ...
            expect(consoleSpy.error).toHaveBeenCalledWith(expect.stringMatching(/An error occurred/), error);
        });
        it('应该能够记录堆栈信息', () => {
            const error = new Error('Test error');
            logger.error('An error occurred', error);
