...
export class VRManager {
    constructor(renderer, camera, scene, options = {}, eventBus) {
        this.xrSession = null;
    ...
    async enterVR() {
        ...
        try {
            ...
            // 获取参考空间
            this.xrRefSpace = await this.xrSession.requestReferenceSpace(this.options.referenceSpaceType);
    ...
    setupControllers() {
        ...
        // 添加控制器模型
        const controllerModelFactory = THREE.XRControllerModelFactory
            ? new THREE.XRControllerModelFactory()
            : null;
     ...
     * 控制器连接
     */
    onControllerConnected(event, controllerIndex) {
     ...
     * 控制器断开
     */
    onControllerDisconnected(controllerIndex) {
...
class _XRControllerModelFactory {
    createControllerModel(_controller) {
        // 创建简单的控制器视觉表示
...
//# sourceMappingURL=VRManager.js.map