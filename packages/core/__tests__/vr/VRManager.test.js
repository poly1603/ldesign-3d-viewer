...
describe('VRManager', () => {
    ...
    beforeEach(() => {
        ...
        globalThis.XRWebGLLayer = MockXRWebGLLayer;
    ...
    describe('VR 支持检测', () => {
        ...
        it('isSessionSupported 抛出错误时应该返回 false', async () => {
            ;
            globalThis.navigator.xr.isSessionSupported = vi.fn().mockRejectedValue(new Error('Not supported'));
            const supported = await VRManager.isVRSupported();
            expect(supported).toBe(false);
        });
        it('不支持 immersive-vr 时应该返回 false', async () => {
            ;
            globalThis.navigator.xr.isSessionSupported = vi.fn().mockResolvedValue(false);
            const supported = await VRManager.isVRSupported();
            expect(supported).toBe(false);
        });
    });
    ...
    describe('进入 VR 模式', () => {
        ...
        it('已经在 VR 中时应该直接返回', async () => {
            await vrManager.enterVR();
            const requestSessionSpy = vi.spyOn(globalThis.navigator.xr, 'requestSession');
            await vrManager.enterVR();
            // 不应该再次请求会话
        ...
        it('请求会话失败时应该抛出错误', async () => {
            ;
            globalThis.navigator.xr.requestSession = vi.fn().mockRejectedValue(new Error('Session request failed'));
    ...
    describe('控制器管理', () => {
        ...
        it('控制器事件应该被正确设置', async () => {
            ...
            renderer.xr.getController = vi.fn(() => mockController);
            const vr = new VRManager(renderer, camera, scene, { controllers: true }, eventBus);
            await vr.initialize();
            await vr.enterVR();
            // 应该为每个控制器添加4个事件监听器
    ...
    describe('边界情况', () => {
        ...
        it('应该能够处理没有 XRControllerModelFactory 的情况', async () => {
            ...
            const originalFactory = THREE.XRControllerModelFactory;
            delete THREE.XRControllerModelFactory;
            const vr = new VRManager(renderer, camera, scene, { controllers: true }, eventBus);
            await vr.initialize();
            // 应该不抛出错误
            ...
            // 恢复
            if (originalFactory) {
                THREE.XRControllerModelFactory = originalFactory;
            }
        });
...
//# sourceMappingURL=VRManager.test.js.map