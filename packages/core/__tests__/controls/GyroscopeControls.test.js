...
describe('GyroscopeControls', () => {
    ...
    describe('权限请求 (iOS 13+)', () => {
        ...
        it('权限请求出错时应该返回 false', async () => {
            ;
            window.DeviceOrientationEvent.requestPermission = vi.fn().mockRejectedValue(new Error('Permission error'));
            const result = await controls.enable();
            expect(result).toBe(false);
            expect(controls.isEnabled()).toBe(false);
        });
    ...
    describe('边界情况', () => {
        ...
        it('应该能够处理极端的角度值', async () => {
            ...
            for (const values of extremeValues) {
                const event = new window.DeviceOrientationEvent('deviceorientation', values);
                // Should not throw error
                expect(() => window.dispatchEvent(event)).not.toThrow();
            }
        });
...
//# sourceMappingURL=GyroscopeControls.test.js.map