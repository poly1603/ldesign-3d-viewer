...
describe('TextureCache', () => {
  ...
  describe('纹理加载', () => {
    ...
    it('应该支持进度回调', async () => {
      ...
      const mockTexture = new THREE.Texture()
      mockTexture.image = { width: 256, height: 256 } as HTMLImageElement
  ...
  describe('缓存清空', () => {
    it('应该能够清空所有纹理', async () => {
      ...
      mockTexture1.image = { width: 256, height: 256 } as HTMLImageElement
      mockTexture1.dispose = vi.fn()

      const mockTexture2 = new THREE.Texture()
      mockTexture2.image = { width: 512, height: 512 } as HTMLImageElement
  ...
  describe('LRU 驱逐策略', () => {
    it('超过最大大小时应该驱逐最旧的纹理', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        tex.image = { width: 256, height: 256 } as HTMLImageElement // ~256KB each
        tex.dispose = vi.fn()
        textures.push(tex)
      }
    ...
    it('访问纹理应该更新其访问时间', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        const tex = new THREE.Texture()
        tex.image = { width: 256, height: 256 } as HTMLImageElement
        textures.push(tex)
      }
