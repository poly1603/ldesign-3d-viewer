...
describe('TouchControls', () => {
  ...
  describe('单指触摸', () => {
    ...
    it('应该在垂直方向限制旋转角度', () => {
      ...
      // 大幅度向上移动（超过限制）
      const touchMove = new TouchEvent('touchmove', {
        touches: [{ clientX: 100, clientY: -10000 } as Touch],
        cancelable: true,
      })
      domElement.dispatchEvent(touchMove)
  ...
  describe('双指触摸', () => {
    ...
    it('应该限制FOV在合理范围内', () => {
      ...
      const touchStart = new TouchEvent('touchstart', {
        touches: [
          { clientX: 100, clientY: 100 } as Touch,
          { clientX: 110, clientY: 100 } as Touch,
        ],
        cancelable: true,
      })
      domElement.dispatchEvent(touchStart)
      ...
      const touchMove = new TouchEvent('touchmove', {
        touches: [
          { clientX: 0, clientY: 100 } as Touch,
          { clientX: 10000, clientY: 100 } as Touch,
        ],
        cancelable: true,
      })
      domElement.dispatchEvent(touchMove)
    ...
    it('捏合手势应该缩小视野', () => {
      ...
      const touchStart = new TouchEvent('touchstart', {
        touches: [
          { clientX: 100, clientY: 100 } as Touch,
          { clientX: 300, clientY: 100 } as Touch,
        ],
        cancelable: true,
      })
      domElement.dispatchEvent(touchStart)
  ...
  describe('性能', () => {
    it('应该快速处理大量触摸事件', () => {
      ...
      for (let i = 0; i < 100; i++) {
        const touchMove = new TouchEvent('touchmove', {
          touches: [{ clientX: i * 2, clientY: 100 } as Touch],
          cancelable: true,
        })
        domElement.dispatchEvent(touchMove)
      }
