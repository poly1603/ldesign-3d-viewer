...
import { definePlugin, Plugin, PluginContext } from '../PluginManager';
...
export const SharePlugin = definePlugin({
  ...
  async shareToTwitter(context: PluginContext): Promise<void> {
    const screenshot = context.viewer.screenshot();
  ...
  async shareToFacebook(context: PluginContext): Promise<void> {
    const url = window.location.href;
  ...
  async shareToWeChat(context: PluginContext): Promise<void> {
    // 生成二维码逻辑
    alert('Scan QR code with WeChat to share');
