...
describe('EventBus', () => {
  ...
  describe('waitFor Promise接口', () => {
    ...
    it('超时应该拒绝Promise', async () => {
      await expect(
        eventBus.waitFor('image:loaded', 50),
    ...
    it('应该能够在超时前正常返回', async () => {
      ...
      await expect(
        eventBus.waitFor('viewer:dispose', 100),
  ...
  describe('边界情况', () => {
    it('触发未订阅的事件不应该报错', () => {
      expect(() => {
        eventBus.emit('non:existent:event' as any)
      }).not.toThrow()
    })
    ...
    it('应该支持自定义事件名（通过索引签名）', () => {
      const handler = vi.fn()
      eventBus.on('custom:event' as any, handler)
      eventBus.emit('custom:event' as any, { data: 'test' })
